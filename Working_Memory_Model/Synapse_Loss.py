"""
Created on Sat Aug  8 14:06:31 2020
...
def calculate(synapses = 2250000, day_step_size = 5, tot_days = 5000, gimme_days = False, gimme_frames = False, export = False):
    ...
    for t in range(0, int(tot_days + 1)): #Goes from 0 to 1000
    	if t == 0:
    ...
    final_list = []
    for step in range(0, int(total_steps + 1)): #Here, the step size is 5
    ...
    days = data_final['Day Number'].tolist()
    if gimme_days and gimme_frames:
        return days, data_final
    elif gimme_days:
        return days
    elif gimme_frames:
        return data_final
